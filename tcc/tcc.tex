...
\begin{document}
  ...
  \begin{figure}[!h]
    \centering
      ...
      \begin{tikzpicture}[scale=.5]
        \def\w{2*pi}
        \def\z{-5.2}
        \draw (-2*pi,0) -- (\w,0) node [right] {GND};
        \draw (0,\z) -- (0,2*pi);
        \draw[step=1,help lines] (-2*pi,-5.2) grid (\w,2*pi);
        \draw[domain=-2*pi:2*pi, samples=100, very thick] plot (\x,{5*cos(\x r)}) node [fill=white, right] {sem {\it offset}};
        \draw[domain=-2*pi:\w, samples=100, very thick, dotted] plot (\x,{5*cos(\x r)+1}) node [fill=white, right] {com {\it offset}};
    \end{tikzpicture}
    \caption{Exemplo de um sinal sem e com {\it offset}.}
  \end{figure}
  ...
  consequentemente
  \[
    V_{\rm IC1A(out)}=V_{\rm IC1A(out)}=V_{\rm IC2A(in)}.
  \]
