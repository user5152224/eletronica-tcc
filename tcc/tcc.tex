...
\begin{document}
  ...
  O {\it offset} de um aplificador \'e o desvio/erro de tens\~ao que um \ampop\ tem em rela\c c\~ao a um \ampop\ ideal equivalente. Por exemplo, ao se injetar nas entradas um potencial correspondente ao GND, \'e esperado que a tens\~ao de sa\'ida seja nula relativa ao GND. Entretanto, em um \ampop\ real a tens\~ao de sa\'ida n\~ao \'e em geral $0\,$V. A diferen\c ca entre o valor esperado idealmente e o valor real \'e o que chamamos de {\it offset}.
  \begin{figure}[!h]
    \centering
      ...
      \begin{tikzpicture}[scale=.5]
        ...
        \draw (\w,\z) node [below] {$1\,{\rm V}/{\rm div}\atop 1\,{\rm s}/{\rm div}$};
  ...
  \[
    V_{\rm out}=G_d(V_+-V_-),
  \]
  em que $G_d$ \'e o ganho diferencial, $V_-$ a tens\~ao na entrada inversora e $V_+$ a tens\~ao na entrada n\~ao-inversora. Todavia, em um \ampop\ real h\'a uma parcela adicional proporcional ao ent\~ao chamado {\bf ganho de modo-comum}, denotado por $G_c$. Em verdade, a tens\~ao de sa\'ida de um \ampop\ diferencial mais realista \'e definida por
  \[
    V_{\rm out}=G_d(V_+-V_-)+G_c\Bigl({V_++V_-\over 2}\Bigr).
    \label{saida_ampop_real}
  \]
  Referindo-se a \cite{HH}, um \ampop\ diferencial \'e aquele cuja sa\'ida \'e inteiramente independente dos sinais individuais, importando apenas a diferen\c ca entre eles. Por\'em, como se nota em (\ref{saida_ampop_real}), $V_{\rm out}$ depende da tens\~ao dos sinais injetados nas entradas, a saber, da sua m\'edia aritm\'etica, e n\~ao apenas de sua diferen\c ca. \`A vista disto, em aplica\c c\~oes de \ampop{s} diferenciais deseja-se que o comportamento seja o mais pr\'oximo de um \ampop\ ideal, isto for\c ca o projetista a tornar o m\'odulo de $G_c$ muito pequeno, em outros termos o mais pr\'oximo de 0. Destarte, quanto menor $|G_c|$, tanto menor ser\'a sua suscetibilidade a {\bf ru\'idos correlacionados}, ou seja a ru\'idos que aparecem com a mesma fase e amplitude em $V_-$ e $V_+$. Na eletr\^onica existe uma grandeza que mensura a imunidade do \ampop\ diferencial a ru\'idos correlacionados, \'e o {\bf CMRR} ({\it Common-Mode Rejection Ratio}\/), em portugu\^es {\bf raz\~ao de rejei\c c\~ao de modo-comum}, doravante a referiremos simplesmente por CMRR. Ela \'e definida por
  \[
    \cmrr=\Bigl|{G_d\over G_c}\Bigr|.
  \]
  Tamb\'em \'e comum v\^e-la expressa em \db
  \[
    \cmrr(\db)=20\cdot\log_{10}\Bigl|{G_d\over G_c}\Bigr|\;\db.
    \label{cmrr_db}
  \]
  Em conformidade, quanto maior a CMRR, melhor ser\'a o amplificador diferencial. Em vista de (\ref{cmrr_db}) um \ampop\ diferencial ideal tem $\cmrr=\infty$. Desta maneira, os projetistas, com a finalidade de obter a melhor qualidade sonora, isto \'e, menor depend\^encia dos ru\'idos, ter\~ao de maximizar o CMRR. 
  ...
  Os resistores de $10\ohm$ $R_{32}$ e $R_{33}$ servem para isolar IC5A e IC5B, previnindo que os sinais provindos das sa\'idas destes amplificadores interajam de forma destrutiva. 
