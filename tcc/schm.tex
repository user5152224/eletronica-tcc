    ...
    \begin{circuitikz}[scale=.6, rotate=90, transform shape]
      ...
      \foreach \a in {0,1} {
        \foreach \y in {0,...,3} {
          ...
          \pgfmathsetmacro{\s}{int(\a*8+40+2*\y)}

          % ICXA
          \path coordinate (NINIC\z A) at (\a,-2*\y*.8*\dmv);
          \draw (NINIC\z A) to [short] ++(6*\a+\per*\dmh,0) node [op amp, anchor=+, noinv input up] (IC\y A) {IC\z A};
          ...
          % ICXB
          \path coordinate (NINIC\z B) at (\a,{-(1+2*\y)*.8*\dmv});
          \draw (NINIC\z B) to [short] ++(6*\a+\per*\dmh,0) node [op amp, anchor=+, noinv input up] (IC\y B) {IC\z B};
